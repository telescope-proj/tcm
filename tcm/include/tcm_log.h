...
#define tcm_fi_error(msg, code)                                                \
    tcm__log_error("[fabric] %s: %s (%d)", msg, fi_strerror(tcm_abs(code)),    \
                   code)
...
#define TCM_FNAME_ (strrchr("/" __FILE__, '/') + 1)
...
#define tcm__log_trace(...)                                                    \
    tcm__log_log(TCM__LOG_TRACE, TCM_FNAME_, __LINE__, __VA_ARGS__)
#define tcm__log_debug(...)                                                    \
    tcm__log_log(TCM__LOG_DEBUG, TCM_FNAME_, __LINE__, __VA_ARGS__)
#define tcm__log_info(...)                                                     \
    tcm__log_log(TCM__LOG_INFO, TCM_FNAME_, __LINE__, __VA_ARGS__)
#define tcm__log_warn(...)                                                     \
    tcm__log_log(TCM__LOG_WARN, TCM_FNAME_, __LINE__, __VA_ARGS__)
#define tcm__log_error(...)                                                    \
    tcm__log_log(TCM__LOG_ERROR, TCM_FNAME_, __LINE__, __VA_ARGS__)
#define tcm__log_fatal(...)                                                    \
    tcm__log_log(TCM__LOG_FATAL, TCM_FNAME_, __LINE__, __VA_ARGS__)
...
void         tcm__log_set_color_mode(int mode);
